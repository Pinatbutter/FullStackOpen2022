  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type='submit'>create</button> <button type='reset' value='' onClick={reset}>reset</button>
      </form>
    </div>
  )
...
const Login = ({users, setLogIn}) => {
  const username = useField('text')
  const password = useField('password')
  const navigate = useNavigate()
  const onSubmit = (event) => {
    event.preventDefault()
    if(users.find(n=> n.user === username.value && n.password === password.value)) {
     setLogIn({user: username.value, password: password.value})
     navigate('/')
    }
  }
  return (
...
const App = () => {
  ...
  const userLogin = (creds) => {
    setLogIn(creds)
    setNotification(`${creds.user} has logged in!`)
    setTimeout(()=>{
      setNotification('')
    }, 5000)
  }
  ...
  return (
    ...
    <h1>Software anecdotes</h1>
      ...
      <Nav className="me-auto">
        ...
        {
          loggedIn? <em>{loggedIn.user} logged in</em> : 
          <Nav.Link href="#" as="span">
     ...
     {(notification && <Alert variant="success"> { notification } </Alert>)}
      <Routes>
        ...
        <Route path="/login" element={<Login users={users} setLogIn={userLogin} />} />
